...
void HAL_I2C_AddrCallback(I2C_HandleTypeDef *hi2c, uint8_t TransferDirection, uint16_t AddrMatchCode){
  if(TransferDirection == I2C_DIRECTION_TRANSMIT){
    ...
    txcount = 0;
    HAL_I2C_Slave_Seq_Transmit_IT(hi2c, sCont.au8I2CSlidePotByteAccess+txcount, 1, I2C_FIRST_FRAME);
  }*/
/*  
  if(TransferDirection == I2C_DIRECTION_TRANSMIT){
    if(is_first_recvd == 0){
...
void HAL_I2C_SlaveTxCpltCallback(I2C_HandleTypeDef *hi2c){
  txcount++;
  HAL_I2C_Slave_Seq_Transmit_IT(hi2c, sCont.au8I2CSlidePotByteAccess+txcount, 1, I2C_NEXT_FRAME);
}
